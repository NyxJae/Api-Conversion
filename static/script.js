...
class APIConverter {
  ...
  setupEventListeners() {
    ...
    if (document.getElementById("channel_provider")) {
      document
        .getElementById("channel_provider")
  ...
  async startDetection() {
    ...
    const requestData = {
      ...
      target_model:
        formData.get("target_model") || formData.get("target_model_select"),
  ...
  displayResults(results) {
    ...
    const supportedCapabilities = Object.values(results.capabilities).filter(
      (cap) => cap.status === "supported"
    ).length;
  ...
  generateCapabilityRows(capabilities) {
    return Object.entries(capabilities)
      .map(([capabilityId, result]) => {
        const statusClass =
          result.status === "supported"
            ? "status-supported"
            : "status-not-supported";
        const statusText =
          result.status === "supported" ? "✅ 支持" : "❌ 不支持";
  ...
  async getAdminAPIKey() {
    try {
      ...
      const result = await response.json();
      if (result.success && result.is_set && result.api_key) {
        return result.api_key;
      }
      return null;
    } catch (error) {
      console.error("获取管理员API Key失败:", error);
      return null;
    }
  }
  ...
  async createChannel() {
    ...
    channelData.weight = parseInt(channelData.weight);

    // 输入验证
    if (!channelData.name || !channelData.provider || !channelData.base_url) {
      showToast("请填写所有必填字段", "error");
      return;
    }

    // 验证权重范围
    if (channelData.weight < 0 || channelData.weight > 1000) {
      showToast("权重必须在0-1000范围内", "error");
      return;
    }
    ...
    if (channelData.use_proxy) {
      ...
      channelData.proxy_port = channelData.proxy_port
        ? parseInt(channelData.proxy_port)
        : null;
    ...
    const channelsHTML = this.channels
      .map((channel) => {
        ...
        const mappingCount = channel.models_mapping
          ? Object.keys(channel.models_mapping || {}).length
          : 0;
...
<p><strong>权重:</strong> <span class="weight-badge">${
  ...
  async testChannel(channelId) {
    try {
      ...
      if (response.ok) {
        showToast(
          `测试成功！响应时间: ${result.test_result.response_time}ms`,
          "success"
        );
      } else {
  ...
  async editChannel(channelId) {
    ...
    const submitButton = document.querySelector(
      '#channelForm button[type="submit"]'
    );
    ...
    document.getElementById("weight").value = channel.weight || 100;
    ...
    if (hasProxy) {
      ...
      if (channel.proxy_password) {
        ...
        const proxyPasswordIcon = document.getElementById(
          "proxy_password_icon"
        );
        if (proxyPasswordIcon) {
          proxyPasswordIcon.innerHTML =
            '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>';
  ...
  async updateChannel(channelId) {
    ...
    if (
      !channelData.api_key ||
      channelData.api_key.trim() === "" ||
      channelData.api_key.includes("***")
    ) {
...
function showToast(message, type = "info") {
  try {
    ...
    toast.style.background =
      type === "success" ? "#e8f5e9" : type === "error" ? "#ffebee" : "#f1f3f5";
    toast.style.border =
      "1px solid " +
      (type === "success"
        ? "#c8e6c9"
